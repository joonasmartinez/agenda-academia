...
export const Horarios = ({Modal, getData, getHoras}) => {
    ...
    return (
        <>
           {data != undefined ? <C.Organize>{}{horarios.map((item, index) => <C.horas onClick={()=>{Modal(true); getData(item)}} key={item}>{item}<C.agendamentos>{reservados[index].length}</C.agendamentos></C.horas> )}</C.Organize> : (<h2>Carregando</h2>)}
        </>
